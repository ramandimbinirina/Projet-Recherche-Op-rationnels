...
function init() {
        ...
        $$(go.Node, "Auto",
            ...
            $$(go.Panel, "Vertical",
                ...
                $$(go.Shape, "Ellipse",
                    ...
                    new go.Binding('fill', 'isCHO', function (s) {
    ...
    diagram.linkTemplate =
        $$(go.Link,  // the whole link panel
            ...
            $$(go.Shape,  // the link shape
                ...
                new go.Binding("stroke", 'isCHO', function (s) {                    
                ...
                if (arc.value != null || arc.value != undefined || arc.value != 0) {
                    ...
                    if (Math.abs(nodes[i].key) > Math.abs(suivants[j].key)) { // si i > j
                        ...
                        if (arc.value == undefined) {
                            ...
                            lamdaV = "λ1 = 0";
                            lamdaNode = `λ${Math.abs(suivants[j].key)} = ${node.data.lamda}`;
                            //lamdaV = `λ${Math.abs(suivants[j].key)}`;
                            
                            potential.push(lamdaNode);
                            appendRow({
                                "i": Math.abs(nodes[i].key),
                                "j": Math.abs(suivants[j].key),
                                "lamdaRes": `λ${Math.abs(suivants[j].key)} - λ${Math.abs(nodes[i].key)} = ${beforeSuivants} - ${beforeNodes} = ${beforeResult}`,
    ...
    function NodeTextBlock() {
        return $$(go.TextBlock,
            {
                font: "bold 11pt helvetica, bold arial, sans-serif",
                alignment: go.Spot.Center, // Centrez le label λ horizontalement
                stroke: "#FC3C00",
                isMultiline: false,  // Assurez-vous que le texte ne soit pas multiligne
                editable: false,  // Empêchez l'édition du texte
                width: 50
            },
            new go.Binding("text", " ", function (s, obj) {
                //console.log("potential : ",potential);
                return nb;
            }).makeTwoWay())
    }
    function createNodeTopTextBlock() {
        return $$(go.TextBlock,  // Ajoutez d'abord le label λ
            {
                font: "bold 11pt helvetica, bold arial, sans-serif",
                alignment: go.Spot.Center, // Centrez le label λ horizontalement
                stroke: "#FC3C00",
